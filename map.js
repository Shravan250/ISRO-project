  ...
  $("#layer").change(function () {
    ...
    $(document).ready(function () {
      $.ajax({
        type: "GET",
        url:
          "http://localhost:8084/geoserver/wfs?service=WFS&request=DescribeFeatureType&version=1.1.0&typeName=" +
          value_layer,
        dataType: "xml",
        success: function (xml) {
...
function query() {
  ...
  var url =
    "http://localhost:8084/geoserver/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=" +
    value_layer +
    "&CQL_FILTER=" +
    value_attribute +
    "%20" +
  ...
  $.getJSON(url, function (data) {
    ...
    caption.innerHTML =
      value_layer + " (Number of Features : " + data.features.length + " )";
...
function clear_all() {
  ...
  map.getView().fit([65.9, 7.48, 98.96, 40.3], {
    duration: 1590,
    size: map.getSize(),
  });
  ...
  if (value !== "None") {
    ...
    if (draw_type.value == "select" || draw_type.value == "clear") {
      ...
      draw1.on("drawend", function (evt) {
        ...
        var url =
          "http://localhost:8084/geoserver/wfs?request=GetFeature&version=1.0.0&typeName=" +
          value_layer +
...
function showLayer(layer) {
  // Implement logic to show specified layers
  console.log(`Showing layer: ${layer}`);
}

function hideLayer(layer) {
  // Implement logic to hide specified layers
  console.log(`Hiding layer: ${layer}`);
}

function navigateToLocation(location) {
  // Implement logic to navigate to the specified location
  console.log(`Navigating to location: ${location}`);
  // Example for demonstration; you should use a geocoding API to get coordinates
  // const coordinates = geocodeLocation(location);
  // map.setView([coordinates.lat, coordinates.lng], 13);
}

function addMarker(details) {
  // Implement logic to add a marker with given details
  console.log(`Adding marker with details: ${details}`);
  // Example implementation
  // const [lat, lng, title] = details.split(',');
  // L.marker([lat, lng]).addTo(map).bindPopup(title);
}

// Start listening for voice commands using TensorFlow.js
